...
```{r eez_layers}
# Load 
EEZs <- vect(file.path(ps_paths$projects, 
                       "prj-MPAs-to-30x30/data/processed/world_EEZs.gpkg")) 

land <- vect(file.path(ps_paths$projects, 
                       "prj-MPAs-to-30x30/data/processed/land.gpkg"))

territorial_seas <- vect(file.path(ps_paths$projects, 
                              "prj-MPAs-to-30x30/data/processed/world_territorial_waters.gpkg"))
...
northern_cyprus <- vect(file.path(prj_path, 
                                  "data/raw/northern_cyprus.gpkg")) |> 
  project("EPSG:4326") |> 
  terra::as.polygons()

# Remove northern cyprus from the 12nm zone

CYP_12nm <- CYP_12nm |> 
  erase(northern_cyprus) 

area_eez <- terra::expanse(CYP_eez)/10^6
area_12nm <- terra::expanse(CYP_12nm)/10^6
area_trnc <- sum(terra::expanse(northern_cyprus)/10^6)
```
...
annual_effort_by_gear |> 
  ...
  theme_custom()+
  labs(y = "", x = "", 
       title = "Fishing Activity in Cyprus' Exclusive Economic Zone (0 - 200 n.m.)", 
...
binned_effort_by_gear <- binned_effort_by_ssvid |> 
  group_by(year, gear_type, lat_bin, lon_bin, pixel_area_km2, in_12nm) |> 
 ...
 #   filter(gear_type == "trawlers", total_fishing_hours > 0) |>
 #   distinct(in_12nm, lat_bin, lon_bin, pixel_area_km2) |>
 #   group_by(in_12nm) |>
 #   summarise(total_area_km2 = sum(pixel_area_km2, na.rm = T))
 # 
 #  binned_effort_by_gear |>
 #    filter(gear_type == "trawlers", total_fishing_hours > 0, in_12nm) |>
 #    group_by(year) |>
 #    summarise(total_area_km2 = sum(pixel_area_km2, na.rm = T)) |>
 #    ungroup() |> mutate(mean(total_area_km2))
```

Overall, bottom trawling has occurred across 4,112 km² of Cyprus’ EEZ and 1,865 km² of its territorial waters, meaning that approximately 4.1% of the EEZ and 24% of the territorial waters have been trawled at least once since 2016 [@gfw2025]. This estimate is based on the cumulative area of 0.01º pixels where trawling has been detected. Using this metric, on average, 1,058 km² of the EEZ and 668 km² of the territorial waters are trawled annually (@fig-effort-by-gear-map).
...
binned_effort_by_gear |> 
  ...
  geom_spatvector(data = CYP_12nm, fill = "transparent", col = "#48A9A6", linetype = "dashed", linewidth = .35)+
  geom_spatvector(data = CYP_eez, fill = "transparent", col = "#E56399", linetype = "dashed", linewidth = .35)+
  geom_spatvector(data = CYP_land, fill = "#646E68", col = "#646E68")+
  geom_spatvector(data = northern_cyprus, fill = "transparent", col = "#000F08", linetype = "dashed")+
  facet_wrap(~gear_type)+
  labs(title = "Fishing Activity in Cyprus' Exclusive Economic Zone (0 - 200 n.m.)",
...
```{r}
#| label: fig-dcf-fleet
#| fig.cap: "Trends in fleet's capacity, power, and age (years) between 2016 and 2024"
#| fig-width: 7
#| fig-height: 4
