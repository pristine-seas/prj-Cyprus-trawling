...
```{r}
carbon_raster <- rast(file.path(ps_paths$projects, 
                                "UCSB-legacy-projects/trawling-co2/data/02_processed_data/carbon_raster.tif")) 
...
c_in_eez <- pull(round(terra::global(carbon_eez_filled, sum, na.rm = T)/10^6)) # 710 million tons of C in territorial waters
...
c_in_12nm <- pull(round(terra::global(carbon_12nm, sum, na.rm = T)/10^6)) # 129 million tons of C in territorial waters

mean_c <- pull(round(terra::global(carbon_12nm, mean, na.rm = T)))
```

To estimate the total carbon stored in the top meter of marine sediment within Cyprus’ territorial waters, we used the global map of sedimentary carbon stocks produced by @atwood2020. Across the Exclusive Economic Zone (EEZ), the total carbon stock is `r c_in_eez` Mt C, while within the 12 n.m. zone, it is estimated at `r c_in_12nm` Mt C (@fig-c-map). Within territorial waters, the average carbon density is approximately `r mean_c` t C/km².
...
ggplot()+
  ...
  paletteer::scale_fill_paletteer_c("pals::parula", 
                                    breaks = c(6000, 8000, 10000,12000),
...
emissions <- co2_remineralized*0.55 # 50% of the CO@ remineralized makes it to the atmosphere (Atwood et al, 2022)
...
Our findings indicate that bottom trawling in Cyprus’ territorial waters disturbs and remineralizes approximately `r round(mean(total_co2_remineralized$total_tons_co2))` tons of CO₂ annually into the water columns. Of this, ~55% ( `r round(mean(total_emissions$total_tCO2))`tons of CO₂ on average) is estimated to be exchanged with the atmosphere within a decade contributing to long-term carbon emissions.
