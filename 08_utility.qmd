...
master_summary <- econ_summary |> 
  ...
  select(year, revenue, total_crew_wage, total_energy_cost, total_repair_cost, total_variable_cost, total_non_variable_cost, total_depreciation_cost,
         CO2_fuel_euro, CO2_sediment_euro, employment_value_euro, protein_value, private_utility, public_utility) 
...
tmp <- avgs_summary |> 
  ...
  mutate(mean = if_else(variable %in% c("CO2_fuel_euro", "CO2_sediment_euro") |
                          str_detect(variable, "cost|wage"), mean*-1, mean)) |> 
  filter(str_detect(variable, "revenue|cost|wage|protein|employment|fuel|prof|util|CO")) |> 
  mutate(variable = fct_relevel(as.factor(str_to_sentence(str_replace_all(str_remove_all(variable, "total_|_euro"), "_", " "))),
                                "Revenue", "Energy cost", "Depreciation cost", "Crew wage", "Repair cost",
                                "Variable cost", "Non variable cost", "Protein value", "Employment value", "Co2 fuel", "Co2 sediment"))

tmp |> 
  mutate(group = case_when(str_detect(variable, "utility") ~ "Net",
                           str_detect(variable, "Co2|Employment|Protein") ~ "Public",
                           TRUE ~ "Private") ,
