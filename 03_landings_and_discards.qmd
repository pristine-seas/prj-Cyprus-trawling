...
avg_landing_by_species <- landing_by_year_and_species |> 
  ...
  summarize(total_landings_tons = sum(landings_tons),
            landings_tons = mean(landings_tons),
            total_discards_tons= sum(discards_tons),
            discards_tons = mean(discards_tons)) |> 
  arrange(desc(total_landings_tons)) 
```

Based on official data, the bottom trawl fishery is dominated by Picarei (*Spicara smaris*), which accounts for over 31 tons annually since 2016, far surpassing any other species. A secondary group contributes moderate landings, including Bogue (*Boops boops*) and Red mullet (*Mullus barbatus*) with 12 and 10 tons repectively, followed by Common Pandora (*Pagellus erythrinus*, 6.8 tons), European squid (*Loligo vulgaris*, 5.2 tons), and Axillary seabream (*Pagellus acarne*, 5.0 tons)(@fig-landings-by-sp-ts). Smaller contributions come from European hake (*Merluccius merluccius*, 1.47 tons) and Striped red mullet (*Mullus surmuletus*, 1.46 tons). Overall, Picarei dominates the fishery, while a handful of other species contribute moderate or minor but steady landings (@fig-landings-by-sp-ts).
...
Common Pandora (*Pagellus erythrinus*) dominates discards, with 2.8 tons recorded since 2016—averaging 0.4 tons per year—far exceeding any other species. Spotted flounder (*Citharus linguatula*) ranks second, with 1.7 tons discarded (0.2 tons annually), but displays a marked temporal shift: it was heavily discarded prior to 2018 (peaking at 1 ton in 2016) and disappears from the discard record thereafter. Notably, it is also entirely absent from landing data. These patterns suggest a potential change in stock availability or catchability (@fig-discards-by-sp-ts). Picarei (*Spicara smaris*), Bogue (*Boops boops*), and Sargo breams (*Diplodus spp.*) follow with 1.6, 1.2, and 1 ton discarded, respectively—each contributing less than 0.2 tons per year on average.
...
discards_by_species <- landing_by_year_and_species |> 
  filter(year > 2015) |> 
  group_by(sp_code, scientific_name, common_name) |> 
  summarize(total_discards_tons = sum(discards_tons),
            discards_tons = mean(discards_tons)) |> 
